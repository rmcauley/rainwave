(function () {
    ...
    for (s = 0; s < arr._shadows.length; s++) {
      ...
      for (i = shadow.arr.length - 1; i >= 0; i--) {
        ...
        if (exists === -1) {
          changed = true;
          (
            shadow.render_delete ||
            arr.render_delete ||
            RWTemplateHelpers.array_item_delete
          )(shadow.el, shadow.arr[i]);
        }
      }
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (exists === -1) {
          ...
          if (
            arr._preserve_item_roots &&
            arr[i].$t &&
            arr[i].$t.item_root &&
            arr[i].$t.item_root.length
          ) {
          ...
          for (i = 0; i < arr.length; i++) {
            for (j = 0; j < arr[i].$t.item_root.length; j++) {
              if (arr[i].$t.item_root[j].parentNode) {
                arr[i].$t.item_root[j].parentNode.appendChild(
                  arr[i].$t.item_root[j],
                );
              }
            }
  ...
  RWTemplateHelpers.array_reconcile = function (fresh, existing) {
    if (
      !existing._unique_field &&
      !existing._update_in_order &&
      !existing._exact_match
    ) {
    ...
    if (existing._update_in_order) {
      ...
      for (j = existing.length - 1; j >= 0; j--) {
        ...
        for (i = fresh.length - 1; i >= 0; i--) {
          if (
            fresh[i][existing._unique_field] ===
            existing[j][existing._unique_field]
          ) {
            found = true;
            break;
          }
      ...
      for (i = 0; i < fresh.length; i++) {
        if (
          existing[i][existing._unique_field] !=
          fresh[i][existing._unique_field]
        ) {
          // eslint-disable-line eqeqeq
  ...
  RWTemplateObject.prototype.update_data = function (newData) {
    ...
    for (var i in newObj) {
      if (!newObj.hasOwnProperty(i)) {
        ...
        if (
          Object.prototype.toString.call(this._c[i]) ===
          Object.prototype.toString.call(newObj[i])
        ) {
          RWTemplateHelpers.array_reconcile(newObj[i], this._c[i]);
        } else {
          console.warn(
            "Mismatching object and error when updating object (new, old, key):",
          );
