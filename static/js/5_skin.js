	...
	that.Extend.MenuPanel = function(menup) {
		...
		menup.draw = function() {
			...
			for (var i = 0; i < menuorder.length; i++) {
				...
				station = createEl("li", { "class": classname, "textContent": " " }, menup.station_list);
			...
			var wmpa = createEl("a", { "href": "/tune_in/" + PRELOADED_SID + ".mp3", "target": "_blank", "onclick": "return false;", "class": "tunein_wmp" }, menup.mp3_download);
			createEl("img", { "width": 16, "height": 16, "src": "static/images/blank.png" }, wmpa);
			wmpa.addEventListener("click", menup.tuneInClickMP3, true);
			var itunesa = createEl("a", { "href": "/tune_in/" + PRELOADED_SID + ".mp3", "target": "_blank", "onclick": "return false;", "class": "tunein_itunes" }, menup.mp3_download);
			...
			var vlca = createEl("a", { "href": "/tune_in/" + PRELOADED_SID + ".mp3", "target": "_blank", "onclick": "return false;", "class": "tunein_vlc" }, menup.mp3_download);
			...
			var winampa = createEl("a", { "href": "/tune_in/" + PRELOADED_SID + ".ogg", "target": "_blank", "onclick": "return false;", "class": "tunein_winamp" }, menup.ogg_download);
			...
			var fb2ka = createEl("a", { "href": "/tune_in/" + PRELOADED_SID + ".ogg", "target": "_blank", "onclick": "return false;", "class": "tunein_fb2k" }, menup.ogg_download);
		...
		menup.showUsername = function(username) {
			...
			if (user.p.user_id <= 1) {
				...
				linkify(createEl("a", { "textContent": _l("managekeys"), "href": "http://rainwave.cc/keys/" }, createEl("li", false, menup.userlinks)));
			}
	...
	*****************************************************************************/
	...
	that.Extend.PlaylistPanel = function(pp) {
		...
		pp.drawArtist = function(wdow, json) {
			...
			for (i = 0; i < json.songs.length; i++) {
				if (json.songs[i].albums[0].id != album_id) {
					if (album.length > 0) {
