...
var RainwavePlayer = (function () {
    ...
    var setupAudioSource = function (i, stream_url) {
        ...
        if (i === streamURLs.length - 1) {
            source.addEventListener("error", function (e) {
                if (self.debug) {
                    console.log(
                        "RainwavePlayer: Error on source " + stream_url
                    );
    ...
    self.useStation = function (station, streamQuery) {
        if (isNaN(parseInt(station)) || !hardcodedStations[parseInt(station)]) {
            if (
                station.toLowerCase &&
                hardcodedStationToSID[station.toLowerCase()]
            ) {
                station = hardcodedStationToSID[station.toLowerCase()];
            } else {
                console.warn(
                    "Unknown Rainwave Station " +
                        station +
                        ", defaulting to All."
                );
                station = 5;
        ...
        streamURLs = [
            hardcodedStations[station] + "." + self.type + streamQuery,
        ];
    };
