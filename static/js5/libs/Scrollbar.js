  ...
  BOOTSTRAP.on_init.push(function (t) {
    ...
    Sizing.add_resize_callback(function () {
      ...
      for (i = 0; i < scrollbars.length; i++) {
        scrollbars[i].el.style.width =
          scrollbars[i].offset_width + scrollbar_width + "px";
  ...
  cls.create = function (scrollable, always_scrollblock, always_hook) {
    ...
    if (enabled || always_scrollblock) {
      ...
      if (self.el.className) {
        self.scrollblock.setAttribute(
          "class",
          "scrollblock " + self.el.className
        );
      }
      if (enabled) {
    ...
    self.set_height = function (height) {
      ...
      if (height !== false) {
        ...
        self.scroll_height =
          height !== undefined ? height : self.el.scrollHeight;
      } else {
        self.scroll_height = self.el.scrollHeight;
      }
      self.scroll_top_max = Math.max(
        0,
    ...
    self.refresh = function () {
      if (
        ...
        visible = true;
        var wheight =
          self.offset_height - handle_margin_top - handle_margin_bottom;
        handle_height = Math.floor(
          wheight * (self.offset_height / self.scroll_height)
        );
    ...
    self.reposition = function (e) {
      ...
      var top =
        ...
        (self.offset_height -
          handle_margin_bottom -
          handle_margin_top -
          handle_height);
      handle.style[Fx.transform] =
        "translateX(-8px) translateY(" +
        Math.floor(handle_margin_top + top) +
        "px)";
    ...
    var mouse_move = function (e) {
      var new_scroll_top = Math.floor(
        original_scroll_top + (e.screenY - original_mouse_y) * scroll_per_px
      );
      new_scroll_top = Math.max(
        Math.min(new_scroll_top, self.scroll_top_max),
        0
      );
