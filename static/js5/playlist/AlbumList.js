var AlbumList = function (el) {
    ...
    API.add_callback("all_albums_paginated", function (json) {
        ...
        json.data.forEach(function (album) {
            album.name_searchable = Formatting.make_searchable_string(
                album.name
            );
        });
        self.update(json.data);
        self.$t.loading_bar.style.transform =
            "scaleX(" + json.progress / 100 + ")";
        if (!json.has_more) {
            loading = false;
            self.loaded = true;
    ...
    API.add_callback("outdated_data_warning", function () {
        ...
        if (
            Sizing.simple &&
            !document.body.classList.contains("playlist_album")
        ) {
            self.unload();
        } else {
            self.load();
    ...
    self.update_item_element = function (item) {
        ...
        if (item.rating_user) {
            ...
            item._el.style.backgroundPosition =
                "right " +
                (-(Math.round(Math.round(item.rating_user * 10) / 2) * 28) +
                    6) +
                "px";
        } else {
    ...
    self.sort_by_alpha = function (a, b) {
        ...
        if (sort_unrated_first) {
            if (!self.data[a].rating_user && self.data[b].rating_user)
                return -1;
