var Sizing = (function () {
    ...
    self.trigger_resize = function () {
        ...
        var right_of_timeline =
            index_t.timeline_sizer.offsetLeft +
            index_t.timeline_sizer.offsetWidth;
        var request_width = 90;
        var detail_width = index_t.detail_container.offsetWidth;
        var lists_width = index_t.lists.offsetWidth;
        var pwr_timeline_width =
            self.width - request_width - detail_width - lists_width;
        self.sizeable_area_height =
            self.height - index_t.sizeable_area.offsetTop;
        self.list_height = self.sizeable_area_height - 80;

        index_t.sizeable_area.style.height = self.sizeable_area_height + "px";
        if (self.detail_area) {
            self.detail_area.style.height =
                self.sizeable_area_height - self.detail_header_size - 20 + "px";
        }

        for (var i = 0; i < self.requests_areas.length; i++) {
            self.requests_areas[i].style.height =
                self.sizeable_area_height -
                self.detail_header_size * 2 -
                30 +
                "px";
        }
        ...
        if (document.body.classList.contains("simple")) {
            ...
            index_t.detail_container.style.left =
                pwr_timeline_width + lists_width + "px";
        }

        var size_changed =
            document.body.classList.contains("normal") != is_normal &&
            document.body.classList.contains("small") != is_small;
