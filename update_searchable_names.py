...
for row in db.c.fetch_all("SELECT group_id, group_name FROM r4_groups"):
	db.c.update("UPDATE r4_groups SET group_name_searchable = %s WHERE group_id = %s", (playlist.make_searchable_string(row['group_name']), row['group_id']))

for row in db.c.fetch_all("SELECT artist_id, artist_name FROM r4_artists"):
	db.c.update("UPDATE r4_artists SET artist_name_searchable = %s WHERE artist_id = %s", (playlist.make_searchable_string(row['artist_name']), row['artist_id']))