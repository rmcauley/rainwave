	...
	def post(self):
		...
		for event in events:
			...
			if not self.user.data['radio_perks']:
				break
		self.append(self.return_name, { "elec_id": elec_id, "code": 0, "key": "vote_submitted", "text": "Vote submitted.", "entry_id": self.get_argument("entry_id") })
	...
	def vote(self, entry_id, event, lock_count):
		...
		if self.user.is_anonymous():
			...
			if self.user.data['listener_voted_entry'] and self.user.data['listener_voted_entry'] == entry_id:
				raise APIException(403, "already_voted_for_song", "Already voted for that song.")
			if self.user.data['listener_voted_entry']:
				...
				if not event.add_vote_to_entry(entry_id, -1):
					...
					raise APIException(500, "internal_error", "Internal server error.")
		else:
		...
		if not already_voted and self.user.data['listener_lock'] and self.user.data['listener_lock_sid'] != self.sid:
			raise APIException(403, "user_locked", "User locked to %s for %s more songs." % (config.station_id_friendly[self.user.data['listener_lock_sid']], self.user.data['listener_lock_counter']))
		...
		if not self.user.lock_to_sid(self.sid, lock_count):
			...
			raise APIException(500, "internal_error", "Internal server error. (logged)  User is now locked to station ID %s." % self.sid)
		...
		if not event.add_vote_to_entry(entry_id):
			...
			raise APIException(500, "server_error", "Internal server error.")
