...
class DonationAddTool(api.web.HTMLRequest):
	...
	def get(self):
		...
		self.write("<button onclick=\"window.top.call_api('admin/add_donation', { 'donor_id': document.getElementById('user_id').value, 'amount': document.getElementById('amount').value, 'message': document.getElementById('message').value, 'private': document.getElementById('private').checked });\">Add Donation</button>")
		self.write(self.render_string("basic_footer.html"))
