...
const doSearch = function (e) {
  ...
  let rawRe = Formatting.make_searchable_string(searchText).split('').join('{1}[^ws]?');
  searchRegex = new RegExp(`^(.*?)(${rawRe})`, 'i');
  rawRe = Formatting.make_searchable_string(searchText).split('').join('{1}[^ws]*?');
  searchRegexGreedy = new RegExp(`^(.*?)(${rawRe})`, 'i');
  API.async_get('search', { search: input.value }, searchResult, searchError);
};
...
INIT_TASKS.on_init.push(function (rootTemplate) {
  ...
  input.addEventListener('input', function () {
    ...
    } else if (!input.value && inputContainer.classList.contains('has-value')) {
      inputContainer.classList.remove('has-value');
    }
  });
