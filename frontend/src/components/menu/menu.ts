...
function closeStationSelect(e) {
  if (template.station_select.classList.contains('open')) {
    ...
    template.station_select_header.removeEventListener('click', closeStationSelect);
    template.header.removeEventListener('mouseleave', closeStationSelect);

    template.station_select.removeEventListener('touchstart', stopPropagation);
  ...
  for (var i = 0; i < Stations.length; i++) {
    if (Stations[i].url) {
      ...
      if (Stations.length > 1) {
        ...
        Stations[i].$t.menu_link.addEventListener('mousedown', openStationSelect);
        Stations[i].$t.menu_link.addEventListener('touchstart', openStationSelect);
      }
