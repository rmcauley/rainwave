...
def set_song_rating(song_id, user_id, rating = None, fave = None):
	...
	for album_id in db.c.fetch_list("SELECT album_id FROM r4_song_album WHERE song_id = %s", (song_id,)):
		album_rating = float(db.c.fetch_var("SELECT ROUND(CAST(AVG(song_rating_user) AS NUMERIC), 1) AS user_rating FROM r4_song_album JOIN r4_song_ratings ON (album_id = %s AND r4_song_album.song_id = r4_song_ratings.song_id) "
									  "WHERE user_id = %s",
									  (album_id, user_id)))
		...
		if existing_rating:
			...
			db.c.update("UPDATE r4_album_ratings SET album_rating_user = %s, album_fave = %s WHERE user_id = %s AND album_id = %s",
						(album_rating, album_fave, user_id, album_id))
		else:
			db.c.update("INSERT INTO r4_album_ratings (album_rating_user, album_fave, user_id, album_id) VALUES (%s, %s, %s, %s)",
						(album_rating, album_fave, user_id, album_id))
