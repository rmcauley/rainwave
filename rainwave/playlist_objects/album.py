...
class Album(AssociatedMetadata):
	...
	def load_extra_detail(self, sid):
		...
		self.data['rating_rank_percentile'] = ((num_albums[sid] - self.data['rating_rank']) / self.data['rating_rank']) * 100
		self.data['rating_rank_percentile'] = max(5, min(99, round(self.data['rating_rank_percentile'])))
		self.data['request_rank_percentile'] = ((num_albums[sid] - self.data['rating_rank']) / self.data['rating_rank']) * 100
