from time import time as timestamp
import hashlib

from api.handler_classes.api_handler import APIHandler
from api.handler_classes.api_handler_with_get import APIHandlerWithGet
from api.exceptions import APIException
from api.handle_url import handle_api_url

from common import config
from common.libs import db
from common.db.cursor import get_cursor


@handle_api_url(r"test/create_anon_tuned_in/(\d+)")
class CreateAnonTunedIn(APIHandlerWithGet):
    description = "Creates a fake tune-in record for an anonymous user at 127.0.0.1."
    local_only = True
    sid_required = False
    auth_required = False
    return_name = "create_anon_tuned_in_result"

    async def post(self, sid):
        async with get_cursor() as cursor:
            if (
                await cursor.fetch_var(
                    "SELECT COUNT(*) FROM r4_listeners WHERE listener_ip = '127.0.0.1' AND user_id = 1"
                )
                == 0
            ):
                await cursor.update(
                    "INSERT INTO r4_listeners (listener_ip, user_id, sid, listener_icecast_id) VALUES ('127.0.0.1', 1, %s, 1)",
                    (int(sid),),
                )
                self.append_standard(
                    "dev_anon_user_tunein_ok",
                    "Anonymous user tune in 127.0.0.1 record completed.",
                )
                return
            if (
                await cursor.fetch_var(
                    "SELECT COUNT(*) FROM r4_listeners WHERE listener_ip = '::1' AND user_id = 1"
                )
                == 0
            ):
                await cursor.update(
                    "INSERT INTO r4_listeners (listener_ip, user_id, sid, listener_icecast_id) VALUES ('::1', 1, %s, 1)",
                    (int(sid),),
                )
                self.append_standard(
                    "dev_anon_user_tunein_ok",
                    "Anonymous user tune in ::1 record completed.",
                )
                return
            if (
                await cursor.fetch_var(
                    "SELECT COUNT(*) FROM r4_listeners WHERE listener_ip = 'localhost' AND user_id = 1"
                )
                == 0
            ):
                await cursor.update(
                    "INSERT INTO r4_listeners (listener_ip, user_id, sid, listener_icecast_id) VALUES ('localhost', 1, %s, 1)",
                    (int(sid),),
                )
                self.append_standard(
                    "dev_anon_user_tunein_ok",
                    "Anonymous user tune in localhost record completed.",
                )
                return
            raise APIException(
                "internal_error", "Anonymous user tune in record already exists.", 500
            )
            self.write_rainwave_output()


class TestUserRequest(APIHandlerWithGet):
    description = "Login as a user."
    local_only = True
    sid_required = False
    auth_required = False

    async def post(self, sid):
        async with get_cursor() as cursor:
            user_id = await cursor.fetch_var("SELECT MAX(user_id) FROM phpbb_users")
            if user_id and user_id < 2:
                user_id = user_id + 1
                await cursor.update(
                    "INSERT INTO phpbb_users (username, user_id, group_id) VALUES ('Test"
                    + str(user_id)
                    + "', %s, 5)",
                    (user_id,),
                )
            elif not user_id:
                user_id = 2
                await cursor.update(
                    "INSERT INTO phpbb_users (username, user_id, group_id) VALUES ('Test"
                    + str(user_id)
                    + "', %s, 5)",
                    (user_id,),
                )
            self.set_cookie(config.phpbb_cookie_name + "_u", user_id)
            session_id = await cursor.fetch_var(
                "SELECT session_id FROM phpbb_sessions WHERE session_user_id = %s",
                (user_id,),
            )
            if not session_id:
                session_id = hashlib.md5(bytes(str(timestamp()), "utf-8")).hexdigest()
                await cursor.update(
                    "INSERT INTO phpbb_sessions (session_id, session_user_id) VALUES (%s, %s)",
                    (session_id, user_id),
                )
            self.set_cookie(config.phpbb_cookie_name + "_u", user_id)
            self.set_cookie(config.phpbb_cookie_name + "_sid", session_id)
            await self.execute(user_id, sid)
            self.append_standard(
                "dev_login_ok",
                "You are now user ID %s session ID %s" % (user_id, session_id),
            )
            self.write_rainwave_output()

    def execute(self, user_id, sid):
        pass


@handle_api_url(r"test/login_tuned_in/(\d+)")
class CreateLoginTunedIn(TestUserRequest):
    description = "Creates or uses a user account with a tuned in record and sets the appropriate cookies so you're that user."
    auth_required = False
    sid_required = False
    return_name = "login_tuned_in_result"

    async def execute(self, user_id, sid):
        async with get_cursor() as cursor:
            if (
                await cursor.fetch_var(
                    "SELECT COUNT(*) FROM r4_listeners WHERE user_id = %s", (user_id,)
                )
                == 0
            ):
                await cursor.update(
                    "INSERT INTO r4_listeners (listener_ip, user_id, sid, listener_icecast_id) VALUES ('127.0.0.1', %s, %s, 1)",
                    (user_id, sid),
                )


@handle_api_url(r"test/login_tuned_out/(\d+)")
class CreateLoginTunedOut(TestUserRequest):
    description = "Creates or uses a user account with no tuned in record sets the appropriate cookies so you're that user."
    auth_required = False
    return_name = "login_tuned_out_result"

    async def execute(self, user_id, sid):
        async with get_cursor() as cursor:
            if (
                await cursor.fetch_var(
                    "SELECT COUNT(*) FROM r4_listeners WHERE user_id = %s", (user_id,)
                )
                or 0
            ) > 0:
                await cursor.update(
                    "DELETE FROM r4_listeners WHERE user_id = %s ", (user_id,)
                )
