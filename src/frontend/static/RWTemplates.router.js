...
(function () {
  ...
  self.detect_url_change = function () {
    ...
    if (oldURL != location.href && old.join() != self.current_route.join()) {
      ...
      if (typeof ga == "object")
        ga("send", "pageview", "/" + self.current_route.join("/"));
  ...
  self.open_route = function (typ, id) {
    ...
    if (self.routes.hasOwnProperty(typ)) {
      if (!id && self.routes[typ]._index) {
        ...
        self.routes[typ][id] &&
        (!self.routes[typ][id]._force_reload || !self.routes[typ][id]._loaded)
      ) {
        ...
        var result = self.routes[typ]._load_new(
          id,
          getNewDiv(),
          function (div) {
            if (div) {
