...
class APIServer:
    ...
    def _listen(self, task_id):
        ...
        request_classes.append((r"/api/.*", api_web.web.Error404Handler))
        request_classes.append((r"/api4/.*", api_web.web.Error404Handler))
        request_classes.append((r".*", api_web.web.HTMLError404Handler))

        # Initialize the help (rather than it scan all URL handlers every time someone hits it)
        src_backend.routes.help.sectionize_requests()
    ...
    def start(self):
        src_backend.libs.locale.load_translations()
        src_backend.libs.locale.compile_static_language_files()
