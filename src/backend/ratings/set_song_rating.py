from backend.db.cursor import RainwaveCursorTx
from .update_album_ratings import UpdatedAlbumRating, update_album_ratings
from time import time as timestamp


async def set_song_rating(
    cursor: RainwaveCursorTx, sid: int, song_id: int, user_id: int, rating: float | None
...
async def clear_song_rating(
    cursor: RainwaveCursorTx, sid: int, song_id: int, user_id: int
) -> list[UpdatedAlbumRating]:
