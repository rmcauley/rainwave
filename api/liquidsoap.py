...
def _get_socket(sid):
    ...
    if (
        ...
        and config.has_station(sid, "liquidsoap_socket_path")
        ...
        client.connect(
            (
                config.get_station(sid, "liquidsoap_telnet_host"),
                config.get_station(sid, "liquidsoap_telnet_port"),
            )
        )
    return client
...
def _send_command(sid, cmd):
    if not config.has_station(sid, "liquidsoap_socket_path") and not config.has_station(
        sid, "liquidsoap_telnet_host"
    ):
        return ""
