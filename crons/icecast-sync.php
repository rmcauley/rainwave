...
$iceservers = array(
        ...
        array(name => "Dracoirs", site => "oc", "sid" => 2, url => "http://admin:ic3rc%40st@69.16.138.218:8000/admin/listclients?mount=/ormgas.ogg"),
	...
	if ($xmlurl = fopen($iceserver['url'], "r")) {
		...
		foreach ($clients as $dbid) {
			if (count(preg_grep("/^" . $dbid . "$/", $ids)) == 0) {
				...
				bb_updateDB("UPDATE rw_listeners SET list_purge = TRUE WHERE list_relay = '" . $iceserver['name'] . "' AND list_icecast_id = " . $dbid);
			}
		}
